...
type MezmurSongRowProps = {
  ...
  /** Catalog-style bordered card (See All lists). `saved` = richer Listen tab row. */
  variant?: 'list' | 'catalog' | 'saved';
...
export const MezmurSongRow = memo(function MezmurSongRow({
  ...
  return (
    <>
      <OrthodoxPressable
        style={[styles.row, isCatalog && styles.rowCatalog, isSaved && styles.rowSaved]}
        onPress={onPress}
        ...
        {thumbnailUrl ? (
          <VideoThumbnail
            ...
            width={isSaved ? LISTEN_SAVED_THUMB.width : undefined}
            height={isSaved ? LISTEN_SAVED_THUMB.height : undefined}
            borderRadius={isSaved ? LISTEN_SAVED_THUMB.borderRadius : undefined}
            style={isSaved ? styles.thumbSavedWrap : undefined}
          />
        ) : (
          <View
            style={[
              styles.thumb,
              isSaved && styles.thumbSaved,
        ...
        <View style={styles.copy}>
          <ThemedText
            style={[styles.title, isCatalog && styles.titleCatalog, isSaved && styles.titleSaved]}
            numberOfLines={2}>
            {title}
          </ThemedText>
          {subtitle ? (
      ...
      {useOptionsMenu && audioTrack ? (
        <MezmurSongOptionsSheet
          ...
          playlistKind={audioTrack?.saveKind ?? 'hymn'}
