...
async function main() {
  ...
  if (rows.length === 0) {
    console.log('No rows found in sheet.'); return;
  }

  // Columns: video_id(0), title(1), artist(2), album(3),
  //          youtube_link(4), thumbnail(5), published_at(6), status(7), language(8)
  const approved = [];
  const rejected = [];

  for (const row of rows) {
    const [videoId, title, artist, album, , thumbnail, publishedAt, status, language] = row;
    if (!videoId || !status) continue;
    const s = status.trim().toLowerCase();
    const lang = (language || 'amharic').trim().toLowerCase();
  ...
  if (approved.length > 0) {
    ...
    else console.log(`✓ ${unique.size} approved mezmur synced to Supabase.`);
  }

  // Mark rejected
  if (rejected.length > 0) {
    for (const videoId of rejected) {
      await sb.from('mezmur').upsert({ video_id: videoId, status: 'rejected' }, { onConflict: 'video_id' });
    }
    console.log(`✓ ${rejected.length} rejected mezmur updated.`);
  }
